...
@Config
@Autonomous (name = "Carter", group = "Orion")
public class Carter extends OpMode {
    ...
    public void init() {
        ...
        toPark
                ...
                .addPath(
                        ...
                        new BezierLine(
                                new Point(13.250, 64.488, Point.CARTESIAN),
                                new Point(13.250, 49, Point.CARTESIAN)
                        )
                )
                .setLinearHeadingInterpolation(Math.toRadians(263), Math.toRadians(263));
...
//        }
        ...
        if((timer.elapsedTime() > 3.5 && timer.elapsedTime() < 7)){
            if (fourthTime){
        ...
        if (timer.elapsedTime() > 7 && timer.elapsedTime() < 8.5) {
            target = 10.5;
            extenderTarget = 16;
            follower.followPath(toPark.build());
            intakeServo.setPower(1);
        ...
        if(timer.elapsedTime() > 8.5 && timer.elapsedTime() < 30){
            extenderTarget = 33;
            intakeServo.setPower(1);
        }
